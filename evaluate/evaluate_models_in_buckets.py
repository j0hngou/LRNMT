...
for (lang_name, model_name), dataset in model_data_dict.items():
    ...
    for i in bucket_sizes:
        test_set_i = test_set.filter(lambda x: len(x['translation']['en']) <= i and len(x['translation']['en']) > prev_bucket)
        prev_bucket = i
        tokenized_test_set = test_set_i.map(preprocess_function, batched=True)
        ...
        #save dict to json
        with open(f"eval_results/{model_name}_{lang_name}_metrics_len_{i}.json", "w") as f:
            json.dump(metrics, f)